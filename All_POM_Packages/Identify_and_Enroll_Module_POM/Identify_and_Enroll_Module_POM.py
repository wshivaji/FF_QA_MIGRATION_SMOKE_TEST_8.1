...
class Identify_And_Enroll_POM(web_driver, web_logger):
    ...
    def upload_image_not_enrolled(self, img_path):
        ...
        self.logger.info(f"clicked on upload image icon")
        time.sleep(2)
        file_path = f"{Path(__file__).parent.parent.parent}\\All_Test_Data\\Common_Test_Data\\dataset4\\vip\\1868_20220526-124604.png"
    ...
    def enroll_person(self):
        ...
        self.logger.info(f"clicked on Identify and enroll link")
        time.sleep(web_driver.one_second)
        file_path = f"{Path(__file__).parent.parent.parent}\\All_Test_Data\\Common_Test_data\\dataset4\\ab\\1847_20220526-124543.png"
        self.upload_image(file_path)
    ...
    def verify_Success_message_is_displayed_below_warning_on_Identify_and_enroll_panel(self):
        try:
            ...
            self.logger.info(f"clicked on Identify and enroll link")
            time.sleep(web_driver.two_second)
            img_path = f"{Path(__file__).parent.parent.parent}\\All_Test_Data\\Common_Test_Data\\dataset4\\vip\\1847_20220526-124543.png"
    ...
    def get_img_file_list(self, folder_name):
        try:
            img_folder = f"{Path(__file__).parent.parent.parent}\\All_Test_Data\\Common_Test_data\\dataset4\\{folder_name}"
            print(img_folder)
    ...
    def enroll_5_images(self, folder_name):
        try:
            ...
            for image in ab_folder_list_of_images:
                ...
                # self.logger.info(f"clicked on upload image icon")
                # time.sleep(2)
                file_path = f"{Path(__file__).parent.parent.parent}\\All_Test_Data\\Common_Test_data\\dataset4\\{folder_name}\\{image} "
                self.upload_image(file_path)
                # pyautogui.write(file_path)
                # pyautogui.press('enter')
                # time.sleep(2)
    ...
    def verify_user_able_approve_enrollment(self):
        try:
            ...
            if load_more_button:
                ...
                self.logger.info("load more btn is not visible.")
            ...
            if approve_enrollment_link:
                self.logger.info(f"approve enrollment link visible: {approve_enrollment_link.is_displayed()}")
                self.logger.info(f"approve enrollment link visible: {approve_enrollment_link.is_enabled()}")
                approve_enrollment_link.click()
            else:
                self.logger.info(f"approve enrollment option is visible: {approve_enrollment_link.is_displayed()}")
                self.logger.info("approve enrollment option is not visible or detected.")
            time.sleep(web_driver.two_second)
